...
from timestamp.dependencies.auth import get_client_user
from timestamp.main import app
...
def officer_user():
    """
    Provide an officer user instance for testing.
    Used in unit tests for services and routes that require an officer user.
    ...
    return User(
        ...
        password="hashed::DevPassword4P!",
        email="dev-officer@timestamp.org",
        role=Role.OFFICER,
    )
...
def public_user():
    ...
    return User(
        ...
        email="dev-public@timestamp.org",
        role=Role.PUBLIC,
    )
...
@pytest.fixture
def create_client(
    admin_user: User, regular_user: User, public_user: User, officer_user: User
    ...
    def client_factory(client_name: Union[Role, str]) -> TestClient | None:
        ...
        client_name : Union[Role, str]
            The role of the client to setup (ADMIN, EMPLOYEE, OFFICER, PUBLIC).
        ...
        match client_name:
            ...
            case Role.OFFICER:
                app.dependency_overrides[get_client_user] = lambda: officer_user
                return TestClient(app)
            case Role.EMPLOYEE:
