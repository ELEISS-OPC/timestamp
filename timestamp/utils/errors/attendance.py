...
class NoRecordsFoundError(Exception):
    """Raised when no attendance records are found for a user."""
    ...
    def __init__(self, user_id: Optional[int] = None, email: Optional[str] = None):
        ...
        if email is not None:
            self.message = (
                f"No attendance records found for user with email '{self.email}'."
            )
        else:
            self.message = (
                f"No attendance records found for user with ID {self.user_id}."
            )
        super().__init__(self.message)
